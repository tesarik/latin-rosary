...
- `localStorage` key `ruzenec_state`, schema `{ version, selectedSet, currentStep }`. `selectedSet` is any prayer-set key stored as a plain string; `storage.ts` does only version/shape checks, and `Rosary.resolveInitialState` validates the key against the registries and rebuilds the sequence (rejecting out-of-range steps).
- **Only multi-step sets persist** across a reload: the rosary and the linear devotions (Leonine, St. Bridget) — gated by `isPersistableKey` (`k in MYSTERIES || k in OTHER_PRAYER_SETS`). Single-prayer sets (litanies, ordinary prayers) have no progress to resume and stay session-only.
