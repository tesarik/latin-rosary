...
- The prayer body is language-tagged (`lang="la"` for Latin, the UI locale for the translation) for screen readers + hyphenation. EB Garamond's Latin `locl` forms render **u→v** (Roman style), so the body sets `font-feature-settings: "locl" 0` to keep modern Latin's `u` — see `bodyStyle` in `PrayerCard.tsx`. Don't drop that when editing the body styles, or `Iesus` becomes `Iesvs`. (æ/œ, kerning, ligatures are unaffected — only `locl` is off.)
