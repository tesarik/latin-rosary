...
- `src/rosary/i18n.ts` — UI strings and language state: `SUPPORTED_LOCALES` (cs/en/sk/de/pl), `Locale`, `DEFAULT_LOCALE`, the `STRINGS` table, and `detectLocale()` (first supported tag from `navigator.languages`, else `cs`). Also owns **two** independent `localStorage` preferences: the UI locale (`ruzenec_locale`, via `loadSavedLocale` / `saveLocale`) and the prayer-body language (`ruzenec_prayer_language`, `latin`/`translation`, via `loadSavedShowTranslation` / `saveShowTranslation`) — the card's LA/CZ toggle. Only UI chrome is localized; the Latin prayer texts are not.
...
- `src/rosary/PrayerSections.tsx` — horizontal bead strand for `linear` prayer sets (the Leonine and St. Bridget prayers, and the Litany of the Saints): one bead per step on a faint thread, no text labels — the current step's name shows in the card's label chip. Items carrying a `section` are drawn larger and are tappable to jump; unsectioned sub-steps are small and inert. Props: `sequence`, `currentStep`, `accentColor`, `onJump`, `locale`.
...
- `src/rosary/Flag.tsx` — inline SVG flags for the language picker, one per `Locale`, drawn to a uniform 24×16 viewBox (the Slovak shield is kept deliberately — without it the flag reads as the Russian tricolor). Props: `locale`, `width?`, `height?`. Used by `MysteryMenu`.
...
- `MYSTERIES` — three traditional mystery sets (`radostny` / `bolestny` / `slavny`), each with a display name, accent color, and 5 mystery clauses. (`prayers.ts`)
- `PRAYER_TYPES` — enum of prayer kinds. The rosary core is `SIGN_OF_CROSS`, `CREED`, `OUR_FATHER`, `HAIL_MARY`, `GLORY_BE`, `FATIMA`, `SALVE_REGINA`; the rest of the ~50 members cover the Leonine and St. Bridget prayers, the *Orationes utilissimæ*, and the litanies (`LITANY_*`, with one type per section for the Litany of the Saints). (`prayers.ts`)
- `buildRosarySequence(mysterySet)` — builds the linear 74-step prayer sequence: (`sequence.ts`)
...
- `PRAYERS` — full Latin prayer texts as single-paragraph strings (Salve Regina is the only one with real line breaks for its three sections). (`prayers.ts`)
- `getHailMary(mystery)` — splits an Ave Maria into `before` / `mystery` / `after` so the per-decade mystery clause can be visually highlighted in the accent color. (`prayers.ts`)
...
- `npm test` — Vitest (`vitest run`, config in `vitest.config.ts`; node env, pure logic — no DOM). `src/rosary/prayers.test.ts` asserts the data invariants: `PRAYERS`/`PRAYERS_CS` key parity, every prayer in every set (rosary + linear + ordinary + litanies) resolves to non-empty Latin **and** Czech text, the rosary is 74 steps with the exact unique `beadId` set, and each ordinary prayer builds one step. `src/rosary/navigation.test.ts` covers the navigation/persistence logic: `isPersistableKey` (incl. prototype-key rejection), `resolveInitialState` (resume, non-persistable/out-of-range/negative rejection), `canGoToStep` bounds, `getPrayerSetMeta` kinds (including that a `kind: "linear"` litany is persistable and builds one step per section), and the Litany of the Saints' 15-step shape. Run these after touching prayer data or the session logic.
...
- To add an **ordinary (single) prayer** or a **litany**: add its text to `PRAYERS` / `PRAYERS_CS` (as a new `StaticPrayerType`), then add an entry to `ORDINARY_PRAYERS` (or `LITANIES`, extending `LitanyKey`) whose `build()` returns the one-step sequence. It appears automatically as a link in the matching start-screen group ("Orationes utilissimæ" / "Litaníæ") — no `MysteryMenu`/`Rosary` changes needed. A litany too long for one card can instead declare `kind: "linear"` and have `build()` return one item per section (each with a `section` label, and its own `StaticPrayerType` for the text) — it then gets the `PrayerSections` stepper and is persisted; see the Litany of the Saints and `LITANY_SAINTS_SECTIONS`. Seasonal/labelled sections use `{r}…{/r}` red rubrics (see Theming).
